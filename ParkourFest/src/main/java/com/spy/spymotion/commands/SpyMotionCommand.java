package com.spy.spymotion.commands;

import com.spy.spymotion.SpyMotion;
import com.spy.spymotion.editor.EditorListener;
import com.spy.spymotion.editor.StructureEditMenu;
...
import com.spy.spymotion.util.MessageUtil;
...
/**
 * Main command executor for /sm and /spymotion commands.
 ...
 */
public class SpyMotionCommand implements CommandExecutor {

    private final SpyMotion plugin;
    private final StructureManager structureManager;
    private final EditorListener editorListener;

    public SpyMotionCommand(SpyMotion plugin, StructureManager structureManager, EditorListener editorListener) {
        this.plugin = plugin;
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (!sender.hasPermission("spymotion.admin")) {
        ...
        switch (sub) {
            ...
            case "reload" -> {
                ...
                MessageUtil.success(sender, "SpyMotion configuration reloaded successfully!");
            }
            case "tools" -> {
    ...
    private void showHelp(CommandSender s) {
        ...
        s.sendMessage(Component.text("═════════ SpyMotion Help ═════════").color(NamedTextColor.GOLD).decorate(TextDecoration.BOLD));
        s.sendMessage(Component.text(" /sm tools").color(NamedTextColor.YELLOW).append(Component.text(" - Open standalone visual structure tools").color(NamedTextColor.GRAY)));
        s.sendMessage(Component.text(" /sm reload").color(NamedTextColor.YELLOW).append(Component.text(" - Reload structure configurations").color(NamedTextColor.GRAY)));
        s.sendMessage(Component.text(" /sm structures <spawn|despawn|start|stop|reset>").color(NamedTextColor.YELLOW).append(Component.text(" - Bulk structure controls").color(NamedTextColor.GRAY)));
        s.sendMessage(Component.text(" /sm structure add <LINEAR|ROTATIONAL> [name] [material]").color(NamedTextColor.YELLOW).append(Component.text(" - Create structure").color(NamedTextColor.GRAY)));
        s.sendMessage(Component.text(" /sm structure remove <id>").color(NamedTextColor.YELLOW).append(Component.text(" - Remove structure").color(NamedTextColor.GRAY)));
        s.sendMessage(Component.text(" /sm structure edit <id>").color(NamedTextColor.YELLOW).append(Component.text(" - Open configuration menu").color(NamedTextColor.GRAY)));
        s.sendMessage(Component.text(" /sm structure list").color(NamedTextColor.YELLOW).append(Component.text(" - List all structures").color(NamedTextColor.GRAY)));
        s.sendMessage(Component.text(" /sm structure <start|stop|reset|spawn|despawn> <id>").color(NamedTextColor.YELLOW).append(Component.text(" - Per-structure controls").color(NamedTextColor.GRAY)));
        s.sendMessage(Component.text("══════════════════════════════════").color(NamedTextColor.GOLD));
    }

    // ========== /sm structures <action> ==========
    private void handleStructuresBulk(CommandSender s, String[] a) {
        if (a.length < 2) {
            MessageUtil.error(s, "Usage: /sm structures <spawn|despawn|start|stop|reset>");
            return;
        }
    ...
    private void showStructureHelp(CommandSender s) {
        MessageUtil.info(s, "Usage: /sm structure <add|remove|list|edit|start|stop|reset|setspeed|setrotationaxis|setendpoint|...>");
    }

    private void structureAdd(CommandSender s, String[] a) {
        if (!(s instanceof Player p)) { MessageUtil.error(s, "Player only."); return; }
        if (a.length < 3) { MessageUtil.error(s, "Usage: /sm structure add <LINEAR|ROTATIONAL> [name] [material]"); return; }
    ...
    private void structureControl(CommandSender s, String[] a, String action) {
        if (a.length < 3) { MessageUtil.error(s, "Usage: /sm structure " + action + " <id>"); return; }
    ...
    private void structureRemove(CommandSender s, String[] a) {
        if (a.length < 3) { MessageUtil.error(s, "Usage: /sm structure remove <id>"); return; }
    ...
    private void structureList(CommandSender s) {
        s.sendMessage(Component.empty());
        s.sendMessage(Component.text("═════ SpyMotion Structures ═════").color(NamedTextColor.GOLD).decorate(TextDecoration.BOLD));
        if (structureManager.getStructureIds().isEmpty()) {
            s.sendMessage(Component.text(" No structures created yet. Use /sm tools!").color(NamedTextColor.GRAY));
        } else {
            for (String id : structureManager.getStructureIds()) {
                ...
                Component line = Component.text(" • ").color(NamedTextColor.DARK_GRAY)
                        ...
                        .append(Component.text("[EDIT]").color(NamedTextColor.AQUA).decorate(TextDecoration.BOLD)
                                .clickEvent(ClickEvent.runCommand("/sm structure edit " + id))
                        ...
                        .append(Component.text("[TP]").color(NamedTextColor.GREEN).decorate(TextDecoration.BOLD)
                                .clickEvent(ClickEvent.runCommand("/sm structure tp " + id))
    ...
    private void structureInfo(CommandSender s, String[] a) {
        if (a.length < 3) { MessageUtil.error(s, "Usage: /sm structure info <id>"); return; }
    ...
    private void structureTp(CommandSender s, String[] a) {
        if (!(s instanceof Player p)) { MessageUtil.error(s, "Player only."); return; }
        if (a.length < 3) { MessageUtil.error(s, "Usage: /sm structure tp <id>"); return; }
        MovingStructureData ms = structureManager.getStructureData(a[2]);
        if (ms == null) { MessageUtil.error(s, "Structure not found."); return; }
    ...
    private void structureRename(CommandSender s, String[] a) {
        if (a.length < 4) { MessageUtil.error(s, "Usage: /sm structure rename <id> <new name>"); return; }
        MovingStructureData ms = structureManager.getStructureData(a[2]);
        if (ms == null) { MessageUtil.error(s, "Structure not found."); return; }
    ...
    private void structureSetField(CommandSender s, String[] a, String field) {
        if (a.length < 4) { MessageUtil.error(s, "Usage: /sm structure set" + field + " <id> <value>"); return; }
        MovingStructureData ms = structureManager.getStructureData(a[2]);
        if (ms == null) { MessageUtil.error(s, "Structure not found."); return; }
        try {
            switch (field) {
    ...
    private void structureSetCenter(CommandSender s, String[] a) {
        if (!(s instanceof Player p)) { MessageUtil.error(s, "Player only."); return; }
        if (a.length < 3) { MessageUtil.error(s, "Usage: /sm structure setcenter <id>"); return; }
        MovingStructureData ms = structureManager.getStructureData(a[2]);
        if (ms == null) { MessageUtil.error(s, "Structure not found."); return; }
    ...
    private void structureSetStartPoint(CommandSender s, String[] a) {
        if (!(s instanceof Player p)) { MessageUtil.error(s, "Player only."); return; }
        if (a.length < 3) { MessageUtil.error(s, "Usage: /sm structure setstartpoint <id>"); return; }
        MovingStructureData ms = structureManager.getStructureData(a[2]);
        if (ms == null) { MessageUtil.error(s, "Structure not found."); return; }
    ...
    private void structureSetEndPoint(CommandSender s, String[] a) {
        if (!(s instanceof Player p)) { MessageUtil.error(s, "Player only."); return; }
        if (a.length < 3) { MessageUtil.error(s, "Usage: /sm structure setendpoint <id>"); return; }
        MovingStructureData ms = structureManager.getStructureData(a[2]);
        if (ms == null) { MessageUtil.error(s, "Structure not found."); return; }
    ...
    private void structureAddPoint(CommandSender s, String[] a) {
        if (!(s instanceof Player p)) { MessageUtil.error(s, "Player only."); return; }
        if (a.length < 3) { MessageUtil.error(s, "Usage: /sm structure addpoint <id>"); return; }
        MovingStructureData ms = structureManager.getStructureData(a[2]);
        if (ms == null) { MessageUtil.error(s, "Structure not found."); return; }
    ...
    private void structureRemovePoint(CommandSender s, String[] a) {
        if (a.length < 4) { MessageUtil.error(s, "Usage: /sm structure removepoint <id> <index>"); return; }
        MovingStructureData ms = structureManager.getStructureData(a[2]);
        if (ms == null) { MessageUtil.error(s, "Structure not found."); return; }
        try {
    ...
    private void structureSetPointSpeed(CommandSender s, String[] a) {
        if (a.length < 5) { MessageUtil.error(s, "Usage: /sm structure setpointspeed <id> <index> <speed>"); return; }
        MovingStructureData ms = structureManager.getStructureData(a[2]);
        if (ms == null) { MessageUtil.error(s, "Structure not found."); return; }
        try {
            int idx = Integer.parseInt(a[3]);
    ...
    private void structureSetPointDelay(CommandSender s, String[] a) {
        if (a.length < 5) { MessageUtil.error(s, "Usage: /sm structure setpointdelay <id> <index> <delay seconds>"); return; }
        MovingStructureData ms = structureManager.getStructureData(a[2]);
        if (ms == null) { MessageUtil.error(s, "Structure not found."); return; }
        try {
            int idx = Integer.parseInt(a[3]);
