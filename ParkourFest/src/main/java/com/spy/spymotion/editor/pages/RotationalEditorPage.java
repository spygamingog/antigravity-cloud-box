...
public class RotationalEditorPage extends HotbarPage {

    public RotationalEditorPage(EditorSession session) {
        super(session);
    }
    ...
    public void setup(Player player) {
        ...
        player.getInventory().setItem(2, new ItemBuilder(Material.COMPASS)
                .name("Set Rotation Center", NamedTextColor.GREEN)
                .lore("Right-click to set pivot center where you are standing.")
                .toolId("set_rotation_center").glow().build());
    ...
    public boolean handleClick(Player player, int slot, Action action, int bx, int by, int bz) {
        ...
        switch (slot) {
            ...
            case 2 -> {
                ...
                MessageUtil.success(player, "Rotation Center/Pivot set to your current coordinates! (" + String.format("%.2f, %.2f, %.2f", x, y, z) + ")");
            }
            case 7 -> {
    ...
    @Override
    public String getPageName() { return "Rotator Editor (" + session.getActiveStructureId() + ")"; }
}
